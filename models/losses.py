...
class ULIPWithImageLoss(nn.Module):
    ...
    def forward(self, outputs):
        ...
        if local_batch_size != self.last_local_batch_size:
            self.labels = local_batch_size * utils.get_rank() + torch.arange(local_batch_size, device=pc_embed.device)
        ...
        pc_embed_all, text_embed_all, image_embed_all = utils.all_gather_batch([pc_embed, text_embed, image_embed])
        ...
        loss = (
            F.cross_entropy(logits_per_pc_text, self.labels) + F.cross_entropy(logits_per_text_pc, self.labels)
        ) / 2 + (
