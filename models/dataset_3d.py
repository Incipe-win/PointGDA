...
from ulip_utils.io import IO
from ulip_utils.build import DATASETS
from ulip_utils.logger import *
from ulip_utils.build import build_dataset_from_cfg
import json
    ...
    def __init__(self, config):
        ...
        if self.uniform:
            self.save_path = os.path.join(
                self.root, "modelnet%d_%s_%dpts_fps.dat" % (self.num_category, split, self.npoints)
            )
        else:
            self.save_path = os.path.join(
                self.root, "modelnet%d_%s_%dpts.dat" % (self.num_category, split, self.npoints)
            )
        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                ...
                if self.generate_from_raw_data:
                    print_log(
                        "Processing data %s (only running in the first time)..." % self.save_path, logger="ModelNet"
                    )
                    ...
                    self.save_path = os.path.join(
                        self.root, "modelnet%d_%s_%dpts_fps.dat" % (self.num_category, split, 8192)
                    )
                    print_log("Load processed data from %s..." % self.save_path, logger="ModelNet")
                    if not self.use_10k_pc:
                        print_log(
                            "since no exact points pre-processed dataset found and no raw data found, load 8192 pointd dataset first, if downsampling with fps to {} happens later, the speed is excepted to be slower due to fps...".format(
                                self.npoints
                            ),
                            logger="ModelNet",
                        )
...
class ShapeNet(data.Dataset):
    def __init__(self, config):
        ...
        self.picked_rotation_degrees = [
            (3 - len(str(degree))) * "0" + str(degree) if len(str(degree)) < 3 else str(degree)
    ...
    def __getitem__(self, idx):
        ...
        picked_model_rendered_image_addr = (
            self.rendered_image_addr + "/" + sample["taxonomy_id"] + "-" + sample["model_id"] + "/"
        )
        picked_image_name = (
            sample["taxonomy_id"]
            + "-"
